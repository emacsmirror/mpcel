...
(defun mpcel-version ()
  ...
  (interactive)
  (message "mpcel version 1.2 - written in 2006  by Bourgoin Jean-Baptiste"))
...
(defun mpcel-single-mode (onoroff)
 ...
 (call-process mpcel-mpc-program nil nil nil "single" onoroff)
  (message (concat "Single mode : " onoroff)))
...
;;  Displays current track playing
;; *** 2009-10-26-18:21 Rene ***
(defun mpcel-current-track ()
  "mpcel : displays current track"
  (interactive)
   (message "Now playing : %s"
	   (shell-command-to-string "mpc current")))
...
;; find current track in the playlist
;; *** 2009-10-26-18:22 Rene ***
(defun mpcel-playlist-find-current-track () 
  "mpcel : Finds current track in the playlist"
  ;; Doesn't work correct yet,  there should be only 1 buffer
  ;; be visible
  (interactive)
  (setq track (shell-command-to-string "mpc current")) 
  (mpcel-playlist-print)
  (switch-to-buffer "*mpcel playlist*")
